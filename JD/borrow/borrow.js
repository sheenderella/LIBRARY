...
ipcRenderer.on('reset-status', (event, { borrowId, originalStatus }) => {
    ...
    showNotification('Condition not provided. Status reset to original.', 'error');

});

ipcRenderer.on('condition-submission-success', (event, { borrowId }) => {
    showNotification('Condition submitted successfully!', 'success');
    
    // Optionally, perform additional actions, like updating the status display
    const dropdown = document.querySelector(`.status-dropdown[data-id="${borrowId}"]`);
    if (dropdown) {
        dropdown.disabled = true; // Ensure the dropdown is disabled post-submission
    }
});
